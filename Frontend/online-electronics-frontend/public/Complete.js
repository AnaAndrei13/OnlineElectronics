...
async function checkStatus() {
  ...
  // Dacă plata a reușit, creează comanda în backend
...
function setPaymentDetails(intent) {
  ...
  switch (intent.status) {
    case "succeeded":
      statusText = "Your payment has been successfully processed!";
      iconColor = "#30B130";
      icon = SuccessIcon;
    ...
    case "processing":
      statusText = "Your payment is being processed...";
      iconColor = "#6D6E78";
      icon = InfoIcon;
      break;
  ...
  const intentId = document.querySelector("#intent-id") || document.querySelector("#session-id");
  const intentStatus = document.querySelector("#intent-status") || document.querySelector("#payment-status-text");
  ...
  if (intentId) intentId.textContent = intent.id;
  if (intentStatus) intentStatus.textContent = intent.status;
  
  if (viewDetails) {
    viewDetails.href = `https://dashboard.stripe.com/payments/${intent.id}`;
...
function setErrorState() {
  ...
  if (statusIcon) {
    statusIcon.style.backgroundColor = "#DF1B41";
    statusIcon.innerHTML = ErrorIcon;
  }
  
  if (statusText) {
    statusText.textContent = "Invalid or expired session.";
  }
  
  if (detailsTable) detailsTable.classList.add("hidden");
